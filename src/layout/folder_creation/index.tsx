...
const CustomizedTreeNode = ({ nodeData, data, setData }: CustomizedNode) => {
  ...
  const deleteDraftFolder = useCallback(() => {
    const filteredData = data.filter((item: any) => item.value !== nodeData.value);
  ...
  const saveDraftFolder = useCallback(() => {
    ...
    setData(updatedData);
  }, [data, nodeData, setData, customizedVissibility, folderTitle, selectedVisibility, users]);

  if (nodeData.draft === 1) {
    return (
      <>
        <div className="custom-option__block">
          <Input style={{ flex: 'auto' }} value={folderTitle} onChange={onNameChange} />
          <Select
...
export default CustomizedTreeNode;
